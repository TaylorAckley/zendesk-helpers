...
class TicketGetter {
    constructor(opts) {
        ...
        this._token = opts.token || process.env.ZENDESK_TOKEN;
        this._query = opts.query ? opts.query : undefined;
        ...
        this._sideloads = opts.sideloads ? opts.sideloads : undefined;
    ...
    getPage(pg) {
        ...
        let _auth = this._auth;
        let _query = this._query;
        let _sideloads = this._sideloads;
        let options = {
            method: 'GET',
            url: _endpoint,
            qs: {
    ...
    populate() {
            return new Promise((resolve, reject) => {
                //handle validations
                if (this._query && this._sideloads) {
                    reject('Error: Sideloads are unavailable when paired with a query.  If you are including a query, you cannot include a sideload.');
                }
                this.getTickets()
                    .then((pgCount) => {
